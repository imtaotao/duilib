...
//                }
    ...
    void CPaintManagerUI::SetResourceLoader( Func_LoadResource func )
    {
        s_ResourceLoader = func;
        return;
    }
    ...
    int CPaintManagerUI::LoadRes(LPCTSTR pstrFilename,  BYTE** pVoid )
    {
        if (s_ResourceLoader)
        {
            int nSize = s_ResourceLoader(pstrFilename, pVoid);
            if (nSize > 0)
            {
                return nSize;
            }
        ...
        CDuiString sFile = CPaintManagerUI::GetResourcePath();
        bool bSuccess = false;
        int nSize = LoadResourceFromFile(pstrFilename, pVoid);
        if (nSize > 0)
        {
            return nSize;
        }
        nSize = LoadResourceFromZip(pstrFilename, pVoid);
        if (nSize > 0)
        {
            return nSize;
        }
